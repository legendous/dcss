...
public class UserService{
	...
	public User getUser(String user_id){
		logger.info("登陆，根据用户名、密码获得用户信息-开始");
		
		List userList = null;
		Map argMap = new HashMap();
		argMap.put("user_id", user_id);
		
		userList = this.daoImpl.getList("com.hkbank.SqlConfig.UserSql.getUser", argMap);
		Map userMap = (Map)userList.get(0);
		User user = null;
		if(userMap == null){
			logger.error("登陆，根据用户名、密码获得用户信息-失败：用户名密码错误");
		}else{
			user = new User((String)userMap.get("USER_ID"), (String)userMap.get("USER_NAME")
								, (String)userMap.get("USER_ROLE"), (String)userMap.get("USER_DEPART")
								, (String)userMap.get("USER_TELEPHONE"), (String)userMap.get("USER_MOBILE")
								, (String)userMap.get("USER_PWD"));
		}
		logger.info("登陆，根据用户名、密码获得用户信息-结束");
		return user;
	}
