  ...
  <body class="bg-7">
	...
	function flowInit(json) {
		...
		$.each(jsonData, function(index, objVal) { //遍历对象数组，index是数组的索引号，objVal是遍历的一个对象。  
			...
			//先用一個ajax查询用户任务信息,假设用户任务序号为1
			//var tast_index = "1";
			//statSelectInit(tast_index);
	}
	...
	function addTr(parentId, objVal){
		...
		var table = "<tr id='tr_"+objVal.index+"' >"+
					...
					"<td width='10%' id='td_stat_org_"+objVal.index+"'>"+stat+"</td>"+ 	//为初始化状态下拉框做准备
					"<td width='10%' id='td_stat_"+objVal.index+"'>"+stat+"</td>"+ 	//为初始化状态下拉框做准备
					"<td width='20%'><small>"+objVal.user+"</small></td>"+
					"</tr>";
		$(table).appendTo($("#"+parentId));	
		statSelectInit(objVal.index);
	...
	function statSelectInit(index){	//状态下拉框初始化
		var select = "";
		select = "<select class='chosen-select chosen-transparent form-control' id='input07'>"+
